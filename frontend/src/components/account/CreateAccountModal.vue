  ...
  >
        ...
        <div class="mt-2 flex rounded-lg bg-gray-100 p-1 dark:bg-dark-700" data-tour="account-form-platform">
          ...
          >
            <svg
              ...
              stroke-width="1.8"
            >
              <path stroke-linecap="round" stroke-linejoin="round" d="M5.5 5.5h4.2l4.3 5.8V5.5h4.5v13h-4.2l-4.3-5.8v5.8H5.5v-13z" />
            </svg>
            Grok
          </button>
          <button
      ...
      <div v-if="form.platform === 'grok'">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3" data-tour="account-form-type">
          ...
          >
            ...
            <div>
              <span class="block text-sm font-medium text-gray-900 dark:text-white">API Key</span>
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ t('admin.accounts.types.grokUpstream') }}</span>
            </div>
          </button>
        </div>
      </div>
          ...
          <input
            ...
            :placeholder="
              form.platform === 'openai' || form.platform === 'sora'
                ...
                : form.platform === 'grok'
                  ? 'https://grok.openaiapi.icu'
                : form.platform === 'gemini'
                  ? 'https://generativelanguage.googleapis.com'
                  : 'https://api.anthropic.com'
            "
          />
          <p class="input-hint">{{ form.platform === 'sora' ? t('admin.accounts.soraUpstreamBaseUrlHint') : form.platform === 'grok' ? t('admin.accounts.grok.baseUrlHint') : baseUrlHint }}</p>
        </div>
        <div>
          ...
          <input
            ...
            "
          />
          <p class="input-hint">{{ form.platform === 'grok' ? t('admin.accounts.grok.apiKeyHint') : apiKeyHint }}</p>
        </div>
    ...
    <div v-else class="space-y-5">
      <OAuthAuthorizationFlow
        ...
        :show-proxy-warning="form.platform !== 'openai' && form.platform !== 'grok' && form.platform !== 'sora' && !!form.proxy_id"
...
const oauthStepTitle = computed(() => {
  ...
  if (form.platform === 'grok') return t('admin.accounts.grok.title')
...
const soraOAuth = useOpenAIOAuth() // Reuse OpenAI OAuth flow for Sora upstream
...
const handleSoraValidateST = async (sessionTokenInput: string) => {
  ...
  try {
    for (let i = 0; i < sessionTokens.length; i++) {
      try {
        const tokenInfo = await (oauthClient as any).validateSessionToken?.(sessionTokens[i], form.proxy_id)
        if (!tokenInfo) {
          failedCount++
          errors.push(`#${i + 1}: ${oauthClient.error.value || 'Validation failed'}`)
          oauthClient.error.value = ''
