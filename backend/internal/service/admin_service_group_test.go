...
func TestAdminService_CreateGroup_WithGrokPricing(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
	inputPrice := 2.5
	videoPrice := 0.6
	input := &CreateGroupInput{
		...
		RateMultiplier:        1.0,
		GrokInputPricePerMTok: &inputPrice,
		GrokVideoPrice15S:     &videoPrice,
	}
	group, err := svc.CreateGroup(context.Background(), input)
	require.NoError(t, err)
	require.NotNil(t, group)
	require.NotNil(t, repo.created)
	require.NotNil(t, repo.created.GrokInputPricePerMTok)
	require.NotNil(t, repo.created.GrokVideoPrice15S)
	require.InDelta(t, inputPrice, *repo.created.GrokInputPricePerMTok, 0.0001)
	require.InDelta(t, videoPrice, *repo.created.GrokVideoPrice15S, 0.0001)
}
