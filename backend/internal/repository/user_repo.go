...
func (r *userRepository) UpdateBalanceAndGetTransition(ctx context.Context, id int64, amount float64) (*service.UserBalanceTransition, error) {
	client := clientFromContext(ctx, r.client)
	result := &service.UserBalanceTransition{}
	err := scanSingleRow(ctx, client, `
		...
		RETURNING id, email, balance - $2 AS previous_balance, balance AS current_balance
	`, []any{id, amount}, &result.UserID, &result.Email, &result.PreviousBalance, &result.CurrentBalance)
	if err != nil {
		return nil, translatePersistenceError(err, service.ErrUserNotFound, nil)
	}
	return result, nil
